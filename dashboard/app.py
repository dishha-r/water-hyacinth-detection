...
def run_inference(img_bgr, conf_thres=CONF):
    ...
    if pred is not None and len(pred):
        pred[:, :4] = scale_boxes(tensor.shape[2:], pred[:, :4], img_bgr.shape).round()
        for *xyxy, conf, cls in pred:
            c = int(cls)
            label = f"{names[c]} {conf:.2f}"
            annotator.box_label(xyxy, label, color=colors(c, True))
            detections.append({"class": names[c], "confidence": round(float(conf), 3), "bbox": [int(x) for x in xyxy]})
...
def record(count, source, conf):
    with stats_lock:
        ...
        stats["history"].append({"time": time.strftime("%H:%M:%S"), "count": count, "source": source})
...
def detect():
    ...
    return jsonify({"image": b64, "detections": dets, "count": len(dets)})


@app.route("/webcam_feed")
def webcam_feed():
    conf = float(request.args.get("conf", CONF))
    return Response(gen_frames(conf), mimetype="multipart/x-mixed-replace; boundary=frame")
